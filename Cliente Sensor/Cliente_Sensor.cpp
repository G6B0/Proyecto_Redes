...
private:
    ...
    SensorData generateSensorData() {
        ...
        std::uniform_real_distribution<float> temp_dist(0.0f, 50.0f);

        std::uniform_real_distribution<float> pres_dist(900.0f, 1150.0f);

        std::uniform_real_distribution<float> hum_dist(10.0f, 100.0f);
...
public:
    ...
    bool sendSensorData() {
        ...
        if (select_result <= 0){
            ...
            std::cerr << "Timeout o error esperando ACK" << std::endl;
        ...
        if (bytes_received <=0){
            ...
            std::cerr << " Error recibiendo ACK o conexión cerrada" << std::endl;
        ...
        if (std::string(ack_buffer) != "ACK"){
            ...
            std::cerr << " ACK inválido recibido :" << ack_buffer << std::endl;
